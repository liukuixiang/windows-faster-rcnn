...
namespace caffe {
	...
	void RPNLayer<Dtype>::LayerSetUp(
		...
		int scales_num = layer_param_.rpn_param().scale_size();     //scale的个数
		for (int i = 0; i < scales_num; ++i)
		{
			anchor_scales_.push_back(layer_param_.rpn_param().scale(i));
		}
		int ratios_num = layer_param_.rpn_param().ratio_size();    //ratio的个数
	...
	vector<vector<float> > RPNLayer<Dtype>::ratio_enum(vector<float> anchor){
		vector<vector<float>> result;
		vector<float> reform_anchor = whctrs(anchor);     //(w,h,ctrx,ctry);
		float x_ctr = reform_anchor[2];         
		float y_ctr = reform_anchor[3];
		float size = reform_anchor[0] * reform_anchor[1];       // w*h;
		for (int i = 0; i < ratios_.size(); ++i)
		{
			float size_ratios = size / ratios_[i];        //w*h的缩放
			float ws = round(sqrt(size_ratios));         //
			float hs = round(ws*ratios_[i]);            //
			vector<float> tmp = mkanchor(ws, hs, x_ctr, y_ctr);    //（x1,y1,x2,y2）
			result.push_back(tmp);
		}
		return result;
	}
	...
	void RPNLayer<Dtype>::proposal_local_anchor(){
		...
		{
			caffe_set(step, Dtype(anchors_[i * 4 + 0]), a + (i * 4 + 0) *step);  //anchors_[i * 4 + 0]对 a + (i * 4 + 0) *step地址内容进行初始化
			...
			caffe_axpy(step, Dtype(1), shift_x, a + (i * 4 + 2)*step);   ///shift_x  * Dtype(1)+(a + (i * 4 + 0)*step))
	...
	void RPNLayer<Dtype>::bbox_tranform_inv(){
		...
		for (int i = 0; i < channel / 4; ++i)
		{
			...
			caffe_mul(2 * step, b + (i * 4 + 2)*step, a + (i * 4 + 0)*step, a + (i * 4 + 0)*step);    //softmax 回归
	...
	void RPNLayer<Dtype>::Forward_cpu(
		...
		m_box_->CopyFrom(*(bottom[1]), false, true);   //copy  data
		...
		m_score_->CopyFrom(*(bottom[0]),false,true);      //copy data
		...
		proposal_local_anchor();   //产生anchor box
